...
import { 
  Search, BarChart3, Eraser, Trash2, Eye, User, FileText, 
  Filter, ShieldAlert, Sparkles, Loader2, RefreshCw, Clock, 
  Medal, Calendar, UserCheck, GraduationCap, X, ChevronDown 
} from 'lucide-react';
...
export default function ResultsBoard({ 
    ...
    // Mặc định: Niên học hiện hành
    const [rAcademicYearFilter, setRAcademicYearFilter] = useState<string>(() => getCurrentAcademicYear());

    // Mặc định: Học sinh của tôi (dành cho Giáo viên) hoặc Tất cả (dành cho SuperAdmin)
    const [rScopeFilter, setRScopeFilter] = useState<'mine' | 'all'>(() => isSuperAdmin ? 'all' : 'mine');

    // Lọc theo Lớp học (tùy chọn)
    const [rClassFilter, setRClassFilter] = useState<string>('all');
    ...
    // Danh sách lớp phù hợp với Niên khóa và Khối được chọn
    const relevantClasses = useMemo(() => {
        return classes.filter(c => {
            if (rAcademicYearFilter !== 'all' && c.academicYear !== rAcademicYearFilter) return false;
            if (rGradeFilter !== 'all' && String(c.grade) !== String(rGradeFilter)) return false;
            return true;
        });
    }, [classes, rAcademicYearFilter, rGradeFilter]);
    ...
    const relevantQuizzes = useMemo(() => {
        return quizzes.filter(q => {
            ...
            const matchYear = rAcademicYearFilter === 'all' || !q.academicYear || q.academicYear === rAcademicYearFilter;
            return matchGrade && matchChapter && matchSubject && matchTeacher && matchYear;
        });
    }, [quizzes, rGradeFilter, rChapterFilter, isSuperAdmin, rSubjectFilter, rTeacherFilter, rAcademicYearFilter]);
    ...
    // Nhóm và lọc kết quả điểm số thông minh: Niên học hiện hành, Khối 12, Học sinh do GV quản lý
    const groupedResults = useMemo(() => {
        ...
        const filtered = results.filter(r => {
            ...
            const studentClass = classes.find(c => c.id === user?.classId || (c.name === user?.className && (!c.academicYear || !user?.academicYear || c.academicYear === user?.academicYear)));
            ...
            // 2. Lọc theo Khối lớp (Mặc định Khối 12)
            const matchGrade = rGradeFilter === 'all' || 
                (quiz && quiz.grade === rGradeFilter) || 
                (user && user.grade === rGradeFilter) ||
                (studentClass && studentClass.grade === rGradeFilter);
            if (!matchGrade) return false;

            // 3. Lọc theo Niên học (Mặc định: Năm hiện hành)
            if (rAcademicYearFilter !== 'all') {
                const matchYear = studentAcademicYear === rAcademicYearFilter ||
                    (quiz?.academicYear && quiz.academicYear === rAcademicYearFilter) ||
                    (r.submittedAt && r.submittedAt.startsWith(rAcademicYearFilter.split('-')[0]));
                if (!matchYear) return false;
            }

            // 4. Lọc theo Phân quyền quản lý của Giáo viên (Mặc định: Học sinh của tôi)
            if (rScopeFilter === 'mine' && currentUser?.id) {
                const isStudentCreatedByMe = Boolean(user?.createdById && user.createdById === currentUser.id);
                const isMyClass = studentClass ? (studentClass.createdBy === currentUser.id || (currentUser.fullName && studentClass.teacherName === currentUser.fullName)) : false;
                const isMyQuiz = quiz?.createdBy === currentUser.id;
                if (!isStudentCreatedByMe && !isMyClass && !isMyQuiz) return false;
            }

            // 5. Lọc theo Lớp học
            if (rClassFilter !== 'all') {
                const matchClass = user?.classId === rClassFilter || studentClass?.id === rClassFilter || user?.className === rClassFilter;
                if (!matchClass) return false;
            }

            // 6. Lọc theo Chương & Đề thi
            ...
            if (!matchQuiz) return false;

            // 7. Lọc SuperAdmin (Môn & GV)
            const matchSubject = !isSuperAdmin || rSubjectFilter === 'all' || (
                quiz ? (
        ...
        return sortedGroups;
    }, [results, quizzes, users, classes, rGradeFilter, rChapterFilter, rQuizFilter, rSearch, rAcademicYearFilter, rScopeFilter, rClassFilter, isSuperAdmin, rSubjectFilter, rTeacherFilter, currentUser]);
    ...
    const isFilteredFromDefaults = rAcademicYearFilter !== getCurrentAcademicYear() || rGradeFilter !== '12' || rScopeFilter !== (isSuperAdmin ? 'all' : 'mine') || rClassFilter !== 'all' || rChapterFilter !== 'all' || rQuizFilter !== 'all' || rSearch.trim() !== '';
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-white p-8 rounded-[3rem] border shadow-sm space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-4 mt-1">
                                ...
                                <span className="text-[9px] font-black text-blue-500 uppercase">Hiển thị: {groupedResults.length} nhóm kết quả</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    ...
                    <div className="space-y-1">
                        ...
                        <input 
                            ...
                            className="w-full bg-slate-50 border rounded-2xl p-3.5 text-xs font-black outline-none focus:border-blue-400"
                            value={rSearch}
                            onChange={e => setRSearch(e.target.value)}
                        />
                    </div>

                    {/* 2. Phạm vi quản lý (Mặc định: Học sinh của tôi) */}
                    <div className="space-y-1">
                        <label className="text-[9px] font-black text-blue-600 uppercase ml-2 flex items-center gap-1"><UserCheck size={10}/> Phạm vi</label>
                        <select 
                            className="w-full bg-blue-50/70 border border-blue-200 text-blue-950 rounded-2xl p-3.5 text-xs font-black uppercase outline-none focus:border-blue-400 cursor-pointer shadow-xs" 
                            value={rScopeFilter} 
                            onChange={e => setRScopeFilter(e.target.value as 'mine' | 'all')}
                        >
                    ...
                    {/* 3. Lọc Niên học (Mặc định: Năm hiện hành) */}
                    <div className="space-y-1">
                        ...
                        <select 
                            className="w-full bg-slate-50 border rounded-2xl p-3.5 text-xs font-black uppercase outline-none focus:border-blue-400 cursor-pointer" 
                        ...
                        >
                            ...
                            {availableAcademicYears.map(yr => (
                                <option key={yr} value={yr}>
                                    NIÊN HỌC {yr} {yr === getCurrentAcademicYear() ? '★ (Hiện hành)' : ''}
                                </option>
                            ))}
                        </select>
                    </div>

                    {/* 4. Lọc Khối lớp (Mặc định: Khối 12) */}
                    <div className="space-y-1">
                    ...
                    {/* 5. Lọc Lớp học */}
                    <div className="space-y-1">
                        <label className="text-[9px] font-black text-slate-400 uppercase ml-2 flex items-center gap-1"><GraduationCap size={10}/> Lớp học</label>
                        <select 
                            className="w-full bg-slate-50 border rounded-2xl p-3.5 text-xs font-black uppercase outline-none focus:border-blue-400 cursor-pointer" 
                            value={rClassFilter} 
                            onChange={e => setRClassFilter(e.target.value)}
                        >
                ...
                {/* Các bộ lọc phụ dành cho SuperAdmin / Nâng cao */}
                <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-slate-100">
                    {/* Lọc Môn học (Chỉ Super Admin) */}
                    {isSuperAdmin && (
                        <div className="flex items-center gap-2">
                            <span className="text-[9px] font-black text-amber-800 uppercase">Môn:</span>
                            <select 
                                className="bg-amber-50 border border-amber-200 text-amber-950 rounded-xl px-3 py-1.5 text-[10px] font-black uppercase outline-none cursor-pointer" 
                                value={rSubjectFilter} 
                                onChange={e => { 
                                    setRSubjectFilter(e.target.value); 
                    ...
                    {/* Lọc Giáo viên (Chỉ Super Admin) */}
                    {isSuperAdmin && (
                        <div className="flex items-center gap-2">
                            <span className="text-[9px] font-black text-amber-800 uppercase">GV:</span>
                            <select 
                                className="bg-amber-50 border border-amber-200 text-amber-950 rounded-xl px-3 py-1.5 text-[10px] font-black uppercase outline-none cursor-pointer" 
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select 
                            className="bg-slate-50 border border-slate-200 text-slate-700 rounded-xl px-3 py-1.5 text-[10px] font-black uppercase outline-none cursor-pointer max-w-[260px] truncate" 
                    ...
                    {/* Nút Khôi phục bộ lọc mặc định */}
                    {isFilteredFromDefaults && (
                        <button 
                            onClick={() => {
                                setRAcademicYearFilter(getCurrentAcademicYear());
                                setRGradeFilter('12');
                                setRScopeFilter(isSuperAdmin ? 'all' : 'mine');
                                setRClassFilter('all');
                                setRChapterFilter('all');
                                setRQuizFilter('all');
                                setRSearch('');
                                if (isSuperAdmin) {
                                    setRSubjectFilter('all');
                                    setRTeacherFilter('all');
                                }
                            }}
                            className="px-3 py-1.5 bg-slate-200 text-slate-700 rounded-xl text-[9px] font-black uppercase hover:bg-slate-300 transition-all flex items-center gap-1 shadow-xs ml-auto"
                            title="Khôi phục bộ lọc mặc định (Năm hiện hành + Khối 12 + Học sinh của tôi)"
                        >
            ...
            {/* Bảng danh sách kết quả học tập */}
                        ...
                        {groupedResults.length === 0 ? (
                            <tr>
                                <td colSpan={8} className="py-20 text-center text-slate-400">
                                    ...
                                    <p className="text-xs font-black uppercase text-slate-600">Chưa có dữ liệu điểm số nào phù hợp</p>
                                    <p className="text-[11px] text-slate-400 mt-1">
                                        Hãy kiểm tra lại bộ lọc Khối, Niên khóa hoặc học sinh đã hoàn thành đề thi chưa.
                                    </p>
                                </td>
                            </tr>
                        ) : (
                                    ...
                                    <tr key={gIdx} className={`hover:bg-slate-50 transition-colors group ${isNA ? 'bg-orange-50/20' : ''} ${isSelected ? 'bg-blue-50/50' : ''}`}>
                                        ...
                                        <td className="p-6">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                    <p className="text-sm font-bold text-slate-600 uppercase leading-snug max-w-[220px] truncate">{quiz?.title || 'Đề đã xóa'}</p>
                                                    {quiz?.subject && (
                                                        <span className="text-[9px] font-bold text-slate-400 uppercase">Môn {quiz.subject}</span>
                                                    )}
                                                </div>
                                            </div>
                                        </td>
